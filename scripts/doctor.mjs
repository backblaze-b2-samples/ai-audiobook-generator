...
function checkFfmpeg() {
  // ffmpeg assembles the chapterized M4B master. Per-chapter MP3s still
  // render without it, so a missing binary is a warning (not a hard fail)
  // that lets `pnpm dev` proceed — master assembly will be skipped until
  // ffmpeg is installed.
  const out = tryExec("ffmpeg -version");
  if (!out) {
    warn(
      "ffmpeg not found — M4B master assembly will be skipped",
      "Install ffmpeg (`brew install ffmpeg` on macOS, `apt install ffmpeg` on Debian/Ubuntu). Per-chapter MP3s render fine without it.",
    );
  }
}
