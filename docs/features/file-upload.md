...
## Purpose
Upload files from the browser to Backblaze B2 with real-time progress tracking. This
is the generic upload surface kept from the starter (writes to the `uploads/` prefix);
audiobook manuscripts go through the New Audiobook studio, not this page.
